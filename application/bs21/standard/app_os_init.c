...
#endif
#if defined(CONFIG_SAMPLE_ENABLE) || defined(CONFIG_APP_ENABLE)
#include "app_init.h"
#endif
#ifdef COREMARK_TEST
...
static app_task_attr_t g_app_tasks[] = {
    {"app", NULL, APP_STACK_SIZE, TASK_PRIORITY_APP, NULL,
     (osal_kthread_handler)app_main},
#if (USE_COMPRESS_LOG_INSTEAD_OF_SDT_LOG == NO)
    {"log", NULL, LOG_STACK_SIZE, TASK_PRIORITY_LOG, NULL,
     (osal_kthread_handler)log_main},
#endif
#ifdef AT_COMMAND
    {"at", NULL, AT_STACK_SIZE, TASK_PRIORITY_CMD, NULL,
     (osal_kthread_handler)uapi_at_msg_main},
#endif
#ifdef BGLE_TASK_EXIST
    {"bt", NULL, BT_STACK_SIZE, TASK_PRIORITY_BT, NULL,
     (osal_kthread_handler)bt_thread_handle},
#ifndef DEVICE_ONLY
#ifndef BT_GLE_ONLY
    {"bt_sdk", NULL, BTH_SDK_STACK_SIZE, TASK_PRIORITY_SDK, NULL,
     (osal_kthread_handler)bt_acore_task_main},
#endif
    {"bth_data", NULL, TASK_STAKDEPTH_BTH_RECV, TASK_PRIORITY_BTH_RECV, NULL,
     (osal_kthread_handler)recv_data_task},
    {"bt_service", NULL, BTH_SERVICE_STACK_SIZE, TASK_PRIORITY_SRV, NULL,
     (osal_kthread_handler)btsrv_task_body},
#endif
#endif
    {"test_task", NULL, TEST_TASK_SIZE, TASK_PRIORITY_TEST, NULL,
     (osal_kthread_handler)test_task}, // lh
};
...
void app_os_init(void) {
  ...
  for (uint8_t i = 0; i < M_NUM_TASKS; i++) {
    g_app_tasks[i].task_handle = osal_kthread_create(
        g_app_tasks[i].task_func, g_app_tasks[i].task_arg,
        g_app_tasks[i].task_name, g_app_tasks[i].task_stack);
    if (g_app_tasks[i].task_handle == NULL) {
      panic(PANIC_TASK_CREATE_FAILED, i);
    }
    osal_kthread_set_priority(g_app_tasks[i].task_handle,
                              g_app_tasks[i].task_pri);
  }
  osal_kthread_unlock();
#ifdef TEST_SUITE
#ifdef AT_COMMAND
...
#endif
#ifdef COREMARK_TEST
  uapi_test_suite_add_function("coremark", "Coremark test Function",
                               coremark_test);
#endif
#if (ENABLE_LOW_POWER == YES)
  uapi_test_suite_add_function(
      "mcu_vote_slp", "MCU vote to sleep or not Function", test_mcu_vote_sleep);
#endif
#ifndef DEVICE_ONLY
#ifdef CONFIG_AT_GLE
...
__attribute__((weak)) void app_main(void *unused) {
  ...
  while (1) { // lint !e716 Main Loop
    ...
    oml_pf_log_print0(LOG_BCORE_PLT_DRIVER_REBOOT, LOG_NUM_DEBUG,
                      LOG_LEVEL_INFO, "App main");
#if defined(PM_MCPU_MIPS_STATISTICS_ENABLE) &&                                 \
    (PM_MCPU_MIPS_STATISTICS_ENABLE == YES)
    oml_pf_log_print2(LOG_BCORE_PLT, LOG_NUM_DEBUG, LOG_LEVEL_INFO,
                      "[Mcpu mips statistics] total work time: %dms, total "
                      "idle time: %dms.\r\n",
                      pm_get_total_work_time(), pm_get_total_idle_time());
    PRINT("[Mcpu mips statistics] total work time: %dms, total idle time: "
          "%dms.\r\n",
          pm_get_total_work_time(), pm_get_total_idle_time());
#endif
    uapi_watchdog_kick();
