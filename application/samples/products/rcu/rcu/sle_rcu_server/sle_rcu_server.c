...
static void sle_rcu_uuid_print(sle_uuid_t *uuid)
{
    if (uuid == NULL) {
        sample_print("%s uuid_print,uuid is null\r\n", SLE_RCU_SERVER_LOG);
        return;
    }
    if (uuid->len == UUID_16BIT_LEN) {
        sample_print("%s uuid: %02x %02x.\n", SLE_RCU_SERVER_LOG,
            uuid->uuid[14], uuid->uuid[15]); /* 14 15: uuid index */
    } else if (uuid->len == UUID_128BIT_LEN) {
        sample_print("%s uuid: \n", SLE_RCU_SERVER_LOG); /* 14 15: uuid index */
        sample_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_RCU_SERVER_LOG, uuid->uuid[UUID_BIT0], uuid->uuid[UUID_BIT1],
            uuid->uuid[UUID_BIT2], uuid->uuid[UUID_BIT3]);
        sample_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_RCU_SERVER_LOG, uuid->uuid[UUID_BIT4], uuid->uuid[UUID_BIT5],
            uuid->uuid[UUID_BIT6], uuid->uuid[UUID_BIT7]);
        sample_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_RCU_SERVER_LOG, uuid->uuid[UUID_BIT8], uuid->uuid[UUID_BIT9],
            uuid->uuid[UUID_BIT10], uuid->uuid[UUID_BIT11]);
        sample_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_RCU_SERVER_LOG, uuid->uuid[UUID_BIT12], uuid->uuid[UUID_BIT13],
            uuid->uuid[UUID_BIT14], uuid->uuid[UUID_BIT15]);
    }
}
...
static void ssaps_start_service_cbk(uint8_t server_id, uint16_t handle, errcode_t status)
{
    if (sle_rcu_server_adv_init() != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_rcu_server_init,sle_rcu_server_adv_init fail\r\n",
                     SLE_RCU_SERVER_LOG);
        return;
    }
...
static errcode_t sle_uuid_server_property_add(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle rcu add property fail, ret:%x\r\n", SLE_RCU_SERVER_LOG, ret);
        osal_vfree(property.value);
        return ERRCODE_SLE_FAIL;
    }
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle rcu add descriptor fail, ret:%x\r\n", SLE_RCU_SERVER_LOG, ret);
        osal_vfree(property.value);
        osal_vfree(descriptor.value);
        return ERRCODE_SLE_FAIL;
    }
...
errcode_t sle_rcu_server_add(void)
{
    ...
    sample_print("%s sle rcu add service in\r\n", SLE_RCU_SERVER_LOG);
    ...
    sample_print("%s sle rcu add service, server_id:%x, service_handle:%x, property_handle:%x\r\n",
                 SLE_RCU_SERVER_LOG, g_server_id, g_service_handle, g_property_handle);
    ret = ssaps_start_service(g_server_id, g_service_handle);
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle rcu add service fail, ret:%x\r\n", SLE_RCU_SERVER_LOG, ret);
        return ERRCODE_SLE_FAIL;
    }
    sample_print("%s sle rcu add service out\r\n", SLE_RCU_SERVER_LOG);
    return ERRCODE_SLE_SUCCESS;
}
...
errcode_t sle_rcu_server_send_report_by_uuid(const uint8_t *data, uint8_t len, uint16_t conn_id)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_rcu_server_send_report_by_uuid,ssaps_notify_indicate_by_uuid fail :%x\r\n",
                     SLE_RCU_SERVER_LOG, ret);
        osal_vfree(param.value);
        return ret;
    }
                              ...
                              ssaps_write_request_callback ssaps_write_callback)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_rcu_server_init,sle_rcu_announce_register_cbks fail :%x\r\n",
                     SLE_RCU_SERVER_LOG, ret);
        return ret;
    }
    ret = sle_conn_register_cbks();
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_rcu_server_init,sle_conn_register_cbks fail :%x\r\n", SLE_RCU_SERVER_LOG, ret);
        return ret;
    }
    ret = sle_ssaps_register_cbks(ssaps_read_callback, ssaps_write_callback);
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_rcu_server_init,sle_ssaps_register_cbks fail :%x\r\n", SLE_RCU_SERVER_LOG, ret);
        return ret;
    }
    ret = enable_sle();
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_rcu_server_init,enable_sle fail :%x\r\n", SLE_RCU_SERVER_LOG, ret);
        return ret;
    }
