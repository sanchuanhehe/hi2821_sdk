...
#define BLE_KEYBOARD_TASK_PRIO                  (osPriority_t)(17)
#define BLE_KEYBOARD_TASK_DURATION_MS           10
...
uint8_t g_ble_keyboard_key_map[KEYSCAN_MAX_ROW][KEYSCAN_MAX_COL] = {
...
#else
static uint8_t g_ble_keyboard_key_map[KEYSCAN_MAX_ROW][KEYSCAN_MAX_COL]  = {
    { 0x04, 0x05 },
    { 0x06, 0x07 },
    { 0x08, 0x09 },
};
...
{
    uapi_set_keyscan_value_map((uint8_t **)g_ble_keyboard_key_map, KEYSCAN_MAX_ROW, KEYSCAN_MAX_COL);
#if defined(CONFIG_KEYSCAN_USE_FULL_KEYS_TYPE)
...
static void *ble_keyboard_task(const char *arg)
{
    ...
    while (get_g_connection_state() == GAP_BLE_STATE_DISCONNECTED) {
        osDelay(BLE_KEYBOARD_TASK_DURATION_MS);
    }
...
static void ble_keyboard_entry(void)
{
    ...
    attr.name = "BLEKeyboardTask";
    attr.attr_bits = 0U;
    attr.cb_mem = NULL;
    attr.cb_size = 0U;
    attr.stack_mem = NULL;
    attr.stack_size = BLE_KEYBOARD_TASK_STACK_SIZE;
    attr.priority = BLE_KEYBOARD_TASK_PRIO;
    if (osThreadNew((osThreadFunc_t)ble_keyboard_task, NULL, &attr) == NULL) {
        /* Create task fail. */
    }
}
