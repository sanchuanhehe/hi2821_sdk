 ...
 *
 * Description: sle adv config for sle gamepad server. \n
...
/* 广播名称 */
static uint8_t sle_local_name[] = "sle_gamepad_server";
...
static uint16_t sle_set_adv_local_name(uint8_t *adv_data, uint16_t max_len)
{
    ...
    sample_print("%s local_name_len = %d\r\n", SLE_GAMEPAD_SERVER_LOG, local_name_len);
    sample_print("%s local_name: ", SLE_GAMEPAD_SERVER_LOG);
...
static int sle_set_default_announce_param(void)
{
    ...
    sample_print("%s sle_gamepad_local addr: ", SLE_GAMEPAD_SERVER_LOG);
...
static int sle_set_default_announce_data(void)
{
    ...
    sample_print("%s data.announce_data_len = %d\r\n", SLE_GAMEPAD_SERVER_LOG, data.announce_data_len);
    sample_print("%s data.announce_data: ", SLE_GAMEPAD_SERVER_LOG);
    ...
    sample_print("%s data.seek_rsp_data_len = %d\r\n", SLE_GAMEPAD_SERVER_LOG, data.seek_rsp_data_len);
    sample_print("%s data.seek_rsp_data: ", SLE_GAMEPAD_SERVER_LOG);
...
{
    sample_print("%s sle enable callback status:%x\r\n", SLE_GAMEPAD_SERVER_LOG, status);
}

errcode_t sle_gamepad_announce_register_cbks(void)
{
    errcode_t ret;
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_gamepad_announce_register_cbks,register_callbacks fail :%x\r\n",
...
errcode_t sle_gamepad_server_adv_init(void)
{
    errcode_t ret;
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_print("%s sle_gamepad_server_adv_init,sle_start_announce fail :%x\r\n", SLE_GAMEPAD_SERVER_LOG, ret);
        return ret;
    }
    return ERRCODE_SLE_SUCCESS;
}
