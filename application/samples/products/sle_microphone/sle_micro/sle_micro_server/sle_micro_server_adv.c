 ...
 *
 * Description: sle adv config for sle micro server. \n
...
static uint16_t sle_set_adv_local_name(uint8_t *adv_data, uint16_t max_len)
{
    ...
    sample_at_log_print("%s local_name_len = %d\r\n", SLE_MICRO_SERVER_LOG, local_name_len);
    sample_at_log_print("%s local_name: ", SLE_MICRO_SERVER_LOG);
...
static int sle_set_default_announce_param(void)
{
    ...
    sample_at_log_print("%s sle_micro_local addr: ", SLE_MICRO_SERVER_LOG);
...
static int sle_set_default_announce_data(void)
{
    ...
    sample_at_log_print("%s data.announce_data_len = %d\r\n", SLE_MICRO_SERVER_LOG, data.announce_data_len);
    sample_at_log_print("%s data.announce_data: ", SLE_MICRO_SERVER_LOG);
    ...
    sample_at_log_print("%s data.seek_rsp_data_len = %d\r\n", SLE_MICRO_SERVER_LOG, data.seek_rsp_data_len);
    sample_at_log_print("%s data.seek_rsp_data: ", SLE_MICRO_SERVER_LOG);
...
{
    sample_at_log_print("%s sle enable callback status:%x\r\n", SLE_MICRO_SERVER_LOG, status);
}

errcode_t sle_micro_announce_register_cbks(void)
{
    errcode_t ret;
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_micro_announce_register_cbks,register_callbacks fail :%x\r\n",
...
errcode_t sle_micro_server_adv_init(void)
{
    errcode_t ret;
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_micro_server_adv_init,sle_start_announce fail :%x\r\n", SLE_MICRO_SERVER_LOG, ret);
        return ret;
    }
    return ERRCODE_SLE_SUCCESS;
}
