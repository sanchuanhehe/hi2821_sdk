 ...
 *
 * Description: sle adv config for sle vdt server. \n
...
#define SLE_CONN_INTV_MIN_DEFAULT                 0x64  // 4--0.5ms
/* 连接调度间隔12.5ms，单位125us */
...
/* 广播名称 */
static uint8_t sle_local_name[NAME_MAX_LENGTH] = " sle_mic_server ";
...
static uint16_t sle_set_adv_local_name(uint8_t *adv_data, uint16_t max_len)
{
    ...
    sample_at_log_print("%s local_name_len = %d\r\n", SLE_VDT_SERVER_LOG, local_name_len);
    sample_at_log_print("%s local_name: ", SLE_VDT_SERVER_LOG);
...
static uint16_t sle_set_adv_data(uint8_t *adv_data, uint16_t length)
{
    ...
    ret = memcpy_s(&adv_data[idx], length - idx, &adv_disc_level, len);
    if (ret != EOK) {
    ...
    ret = memcpy_s(&adv_data[idx], length - idx, &adv_access_mode, len);
    if (ret != EOK) {
...
static uint16_t sle_set_scan_response_data(uint8_t *scan_rsp_data, uint16_t length)
{
    ...
    ret = memcpy_s(scan_rsp_data, length, &tx_power_level, scan_rsp_data_len);
    if (ret != EOK) {
...
static int sle_set_default_announce_param(void)
{
    ...
    sample_at_log_print("%s sle_vdt_local addr: ", SLE_VDT_SERVER_LOG);
...
static int sle_set_default_announce_data(void)
{
    ...
    sample_at_log_print("%s data.announce_data_len = %d\r\n", SLE_VDT_SERVER_LOG, data.announce_data_len);
    sample_at_log_print("%s data.announce_data: ", SLE_VDT_SERVER_LOG);
    ...
    sample_at_log_print("%s data.seek_rsp_data_len = %d\r\n", SLE_VDT_SERVER_LOG, data.seek_rsp_data_len);
    sample_at_log_print("%s data.seek_rsp_data: ", SLE_VDT_SERVER_LOG);
...
{
    sample_at_log_print("%s sle enable callback status:%x\r\n", SLE_VDT_SERVER_LOG, status);
}

errcode_t sle_vdt_announce_register_cbks(void)
{
    errcode_t ret;
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_announce_register_cbks,register_callbacks fail :%x\r\n",
...
errcode_t sle_vdt_server_adv_init(void)
{
    errcode_t ret;
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_adv_init,sle_start_announce fail :%x\r\n", SLE_VDT_SERVER_LOG, ret);
        return ret;
    }
    return ERRCODE_SLE_SUCCESS;
}
