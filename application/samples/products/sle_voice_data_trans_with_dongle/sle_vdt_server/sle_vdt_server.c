 ...
 *
 * Description: SLE VDT Server Source. \n
...
#include "sle_vdt_server_adv.h"
#include "sle_vdt_server.h"
...
static uint8_t g_sle_vdt_base[] = { 0x37, 0xBE, 0xA8, 0x80, 0xFC, 0x70, 0x11, 0xEA, \
...
{
    errcode_t ret;
    ret = memcpy_s(out->uuid, SLE_UUID_LEN, g_sle_vdt_base, SLE_UUID_LEN);
    if (ret != EOK) {
...
static void sle_vdt_uuid_print(sle_uuid_t *uuid)
{
    if (uuid == NULL) {
    ...
    if (uuid->len == UUID_16BIT_LEN) {
        ...
        sample_at_log_print("%s uuid: \n", SLE_VDT_SERVER_LOG); /* 14 15: uuid index */
        sample_at_log_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_VDT_SERVER_LOG, uuid->uuid[0], uuid->uuid[1],
            uuid->uuid[2], uuid->uuid[3]);
        sample_at_log_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_VDT_SERVER_LOG, uuid->uuid[4], uuid->uuid[5],
            uuid->uuid[6], uuid->uuid[7]);
        sample_at_log_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_VDT_SERVER_LOG, uuid->uuid[8], uuid->uuid[9],
            uuid->uuid[10], uuid->uuid[11]);
        sample_at_log_print("%s 0x%02x 0x%02x 0x%02x \n", SLE_VDT_SERVER_LOG, uuid->uuid[12], uuid->uuid[13],
    ...
    errcode_t status)
{
    sample_at_log_print("%s ssaps ssaps_mtu_changed_cbk callback server_id:%x, conn_id:%x, mtu_size:%x, status:%x\r\n",
        SLE_VDT_SERVER_LOG, server_id, conn_id, mtu_size->mtu_size, status);
        g_ssaps_ready = true;
    if (g_sle_pair_hdl == 0) {
    ...
    uint16_t handle, errcode_t status)
{
    sample_at_log_print("%s add property cbk callback server_id:%x, service_handle:%x,handle:%x, status:%x\r\n",
        SLE_VDT_SERVER_LOG, server_id, service_handle, handle, status);
    sle_vdt_uuid_print(uuid);
}
    ...
    uint16_t property_handle, errcode_t status)
{
    sample_at_log_print("%s add descriptor cbk callback server_id:%x, service_handle:%x, property_handle:%x, \
        status:%x\r\n", SLE_VDT_SERVER_LOG, server_id, service_handle, property_handle, status);
    sle_vdt_uuid_print(uuid);
}
...
static errcode_t sle_uuid_server_property_add(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle vdt add property fail, ret:%x\r\n", SLE_VDT_SERVER_LOG, ret);
        osal_vfree(property.value);
        return ERRCODE_SLE_FAIL;
    }
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle vdt add descriptor fail, ret:%x\r\n", SLE_VDT_SERVER_LOG, ret);
        osal_vfree(property.value);
        osal_vfree(descriptor.value);
        return ERRCODE_SLE_FAIL;
    }
...
static errcode_t sle_vdt_server_add(void)
{
    errcode_t ret;
    sle_uuid_t app_uuid = { 0 };

    sample_at_log_print("%s sle vdt add service in\r\n", SLE_VDT_SERVER_LOG);
    ...
    sample_at_log_print("%s sle vdt add service, server_id:%x, service_handle:%x, property_handle:%x\r\n",
        SLE_VDT_SERVER_LOG, g_server_id, g_service_handle, g_property_handle);
    ret = ssaps_start_service(g_server_id, g_service_handle);
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle vdt add service fail, ret:%x\r\n", SLE_VDT_SERVER_LOG, ret);
        return ERRCODE_SLE_FAIL;
    }
    sample_at_log_print("%s sle vdt add service out\r\n", SLE_VDT_SERVER_LOG);
    return ERRCODE_SLE_SUCCESS;
}
...
errcode_t sle_vdt_server_send_report_by_uuid(uint8_t *data, uint16_t len)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_send_report_by_uuid,ssaps_notify_indicate_by_uuid fail :%x\r\n",
            SLE_VDT_SERVER_LOG, ret);
        osal_vfree(param.value);
        return ret;
    }
    ...
    sle_acb_state_t conn_state, sle_pair_state_t pair_state, sle_disc_reason_t disc_reason)
{
    sample_at_log_print("%s connect state changed callback conn_id:0x%02x, conn_state:0x%x, pair_state:0x%x, \
        disc_reason:0x%x\r\n", SLE_VDT_SERVER_LOG,conn_id, conn_state, pair_state, disc_reason);
...
{
    errcode_t ret;
    ret = sle_vdt_announce_register_cbks();
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_init,sle_vdt_announce_register_cbks fail :%x\r\n",
        SLE_VDT_SERVER_LOG, ret);
        return ret;
    }
    ret = enable_sle();
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_init,enable_sle fail :%x\r\n", SLE_VDT_SERVER_LOG, ret);
        return ret;
    }
    ret = sle_conn_register_cbks();
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_init,sle_conn_register_cbks fail :%x\r\n", SLE_VDT_SERVER_LOG, ret);
        return ret;
    }
    osal_mdelay(SLE_SERVER_INIT_DELAY_MS);
    ret = sle_ssaps_register_cbks(ssaps_read_callback, ssaps_write_callback);
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_init,sle_ssaps_register_cbks fail :%x\r\n", SLE_VDT_SERVER_LOG, ret);
        return ret;
    }
    ret = sle_vdt_server_add();
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_init,sle_vdt_server_add fail :%x\r\n", SLE_VDT_SERVER_LOG, ret);
        return ret;
    }
    ret = sle_vdt_server_adv_init();
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_vdt_server_init,sle_vdt_server_adv_init fail :%x\r\n", SLE_VDT_SERVER_LOG, ret);
        return ret;
    }
