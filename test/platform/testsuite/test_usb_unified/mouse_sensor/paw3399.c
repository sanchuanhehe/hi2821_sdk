...
#define MOUSE_3399_READ_TIMES           60
#define MOUSE_3399_READ_REG             0x6c
#define MOUSE_3399_READ_TARGET_VAL      0x80
#define MOUSE_3399_READ_DELAY           1000
...
const spi_mouse_cfg_t g_paw3399db_cfg_1[] = {
    { WRITE, {{ 0x3a, 0x5A }} },
    { DELAY, { .delay = 5000 }},
    { WRITE, {{ 0x40, 0x80 }} },
    { WRITE, {{ 0x7f, 0x0E }} },
...
const spi_mouse_cfg_t g_paw3399db_cfg_2[] = {
    { WRITE, {{ 0x22, 0x00 }} },
    { WRITE, {{ 0x55, 0x00 }} },
    { WRITE, {{ 0x7f, 0x00 }} },
    { WRITE, {{ 0x40, 0x00 }} },
...
static void mouse_sensor_mid_init_1(void)
{
    for (int i = 0; i < MOUSE_3399_READ_TIMES; i++) {
        if (mouse_read_reg(MOUSE_3399_READ_REG) == MOUSE_3399_READ_TARGET_VAL) {
            print("init 0x6c:0x80\r\n");
            return;
        }
        uapi_tcxo_delay_us(MOUSE_3399_READ_DELAY);
    }
    spi_mouse_cfg_t cfg[] = {
        {0x1, {{ 0x7F, 0x14 }} },
        {0x1, {{ 0x6C, 0x00 }} },
    ...
    print("read 0x6c without value 0x80 for 60 times\r\n");
    mouse_opration(cfg, sizeof(cfg) / sizeof(spi_mouse_cfg_t));
}

static void paw3399_chaneg_dpi(void)
{
...
static mouse_freq_type_t paw_3399_mouse_init(void)
{
    print("3399 init\r\n");
    mouse_spi_open();
    mouse_opration(g_paw3399db_cfg_1, sizeof(g_paw3399db_cfg_1) / sizeof(spi_mouse_cfg_t));
    mouse_sensor_mid_init_1();
    mouse_opration(g_paw3399db_cfg_2, sizeof(g_paw3399db_cfg_2) / sizeof(spi_mouse_cfg_t));
...
static void paw3399_get_xy(int16_t *x, int16_t *y)
{
...
high_mouse_oprator_t g_paw3399_operator = {
    .get_xy = paw3399_get_xy,
    .init = paw_3399_mouse_init,
};

high_mouse_oprator_t mouse_get_paw3399_operator(void)
{
    print("return 3399 oprator\r\n");
    return g_paw3399_operator;
}