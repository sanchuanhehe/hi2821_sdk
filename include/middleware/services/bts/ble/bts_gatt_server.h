 ...
 * @endif
 */
typedef void (*gatts_delete_all_service_callback)(uint8_t server_id, errcode_t status);
...
typedef struct {
    ...
    gatts_stop_service_callback stop_service_cb;             /*!< @if Eng Service stoped callback.
                                                                  @else   停止服务回调函数。 @endif */
    gatts_delete_all_service_callback delete_service_cb;         /*!< @if Eng All service deleted callback.
 ...
 * @param  [in]  service_handle Service handle.
 * @param  [in]  character      Characteristic.
 * @param  [out] handle         Characteristic handle.
 * @retval error code.
 * @par Depends:
 * @li bts_def.h
 * @else
