...
* Description: header file for wifi device config. \n
* Author: CompanyName \n
* History: \n
* 2022-02-22，更新文件注释 \n
* 2021-12-31，初始化该文件 \n
...
/**
 * @if Eng
 * @brief  Indicates the maximum length of a WiFi WPS PIN.The maximum length is 8,and the last bit is reserved and set to '\0'.
 * @else
 ...
 */
typedef enum {
    WIFI_WPA_PSK_NOT_USE,   /*!< @if Eng The PSK does not need to be calculated in advance. In this case, the PSK in wifi_fast_connect_stru is not used.
...
typedef enum wifi_security_enum {
    ...
    WIFI_SEC_TYPE_OPEN,                 /*!< @if Eng Open.
                                             @else Open。 @endif */
    WIFI_SEC_TYPE_WEP,                  /*!< @if Eng Wired Equivalent Privacy (WEP).
                                             @else WEP。 @endif */
...
typedef struct wifi_sta_config_stru {
    ...
    wifi_security_enum security_type;               /*!< @if Eng Security type,It is defined in link wifi_security_enum.
                                                         @else 安全类型。 @endif */
    int32_t wifi_psk_type;                          /*!< @if Eng The type of Psk.
                                                         @else Psk的类型。 @endif */
...
typedef struct wifi_fast_connect_stru {
    wifi_sta_config_stru config;                /*!< @if Eng The Wi-Fi station configuration used to connect to a specified Wi-Fi device.
...
typedef struct {
    ...
    uint8_t linkloss_threshold;         /*!< @if Eng Threshold of linkloss.
                                             @else 取值范围: [50, 250]. 设置为n时，表示linkloss阈值基础时间为(100*n)ms。 @endif */
    uint8_t resv[2];
} linkloss_paras_stru;
