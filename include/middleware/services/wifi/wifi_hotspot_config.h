...
* Description: header file for wifi hotspot config. \n
* Author: CompanyName \n
* History: \n
* 2022-02-22，更新文件注释 \n
* 2021-12-31，初始化该文件 \n
 ...
 */
typedef struct {
    uint32_t beacon_interval;         /*!< @if Eng beacon_interval, range(25ms~1000ms), default(100ms), 0 means not configured.
    ...
    uint32_t group_rekey;             /*!< @if Eng group_rekey, range(30s~86400s), default(86400s), 0 means not configured.
                                           @else   config->groupRekey, 范围30s~86400s, 默认86400s，0表示未配置。  @endif */
    uint32_t hidden_ssid_flag;        /*!< @if Eng hidden_ssid_flag, not hidden:1, hidden: 2.The default value is not hidden, 0 means not configured.
                                           @else   hiddenSsidFlag, 不隐藏：1，隐藏：2。默认配置为不隐藏，0表示未配置。  @endif */
    uint32_t gi;                      /*!< @if Eng gi,The default value is auto_GI.0, indicating that this parameter is not configured.If a non-zero valid value is configured, a valid protocol mode must also be configured.This function is not supported currently and will be supported in the future.
                                           @else   config->Gi, 默认是auto_GI.0表示未配置。进行非0有效值配置时，需要同时配置有效的协议模式。当前暂不支持，后续支持。  @endif */
    protocol_mode_enum protocol_mode; /*!< @if Eng protocol_mode_enum mode.By default, this parameter is configured based on the maximum protocol capability of the chip.0: not configured.
