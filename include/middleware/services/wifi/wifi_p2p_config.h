...
* Description: header file for wifi p2p config. \n
* Author: CompanyName \n
* History: \n
* 2022-02-22，更新文件注释 \n
* 2021-12-31，初始化该文件 \n
...
typedef struct wfd_info_stru {
    int16_t deviceInfo;    /*!< @if Eng WFD Device Type.
                              @else   WFD设备类型。  @endif */
    int16_t ctrlPort;      /*!< @if Eng Control port number.
                              @else   控制端口号。  @endif */
    int16_t maxThroughput; /*!< @if Eng Maximum throughput.
                              @else   最大吞吐量。  @endif */
    int8_t reserved[2];
} wfd_info_stru;
...
typedef struct p2p_device_stru {
    ...
    uint8_t enable_add_group : 1;   /*!< @if Eng Indicates whether to allow GC to be added to the group corresponding to the scanned go.
...
typedef struct p2p_config_stru {
    ...
    uint8_t persistent;                    /*!< @if Eng 1: Persistent network storage; 0: not based on permanent network storage.
...
typedef struct p2p_status_info_stru {
    int32_t operation_channel;            /*!< @if Eng Center frequency of the working channel after the GO/GC connection,
    ...
    int8_t group_ssid[WPS_DEV_NAME_LEN];  /*!< @if Eng Group SSID, valid only when wpa_state is set to CONNECTED and P2P WPS succeeds.
                                               @else   Group SSID，并且P2P的WPS关联成功（当前阶段组协商成功）时有效。  @endif */
    uint8_t group_bssid[WIFI_MAC_LEN];    /*!< @if Eng Group BSSID==go bssid, valid only when wpa_state is set to CONNECTED and P2P WPS succeeds.
                                               @else   Group BSSID==go bssid，在wpa_state为CONNECTED，并且P2P的WPS关联成功（当前阶段组协商成功）时有效。  @endif */
    int8_t reserved[3];
...
typedef struct {
    int8_t dev_name[WPS_DEV_NAME_LEN];      /*!< @if Eng Device name.
                                                 @else   设备名称。  @endif */
    uint8_t wps_method;                     /*!< @if Eng WPS connection mode supported.The options are WpsMethod.The options are 0, 1, 2, and 3.
    ...
    int32_t oper_channel;                   /*!< @if Eng Recommended working channel: 0 indicates a random channel.A non-zero valid value indicates the recommended channel.
