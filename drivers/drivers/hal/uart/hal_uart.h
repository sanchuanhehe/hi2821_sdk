...
/**
 * @if Eng
 * @brief  Deinit the hal_uart which will clear the base address of registers has been
 *         set by @ref hal_uart_regs_init.
 * @param  [in]  bus The uart bus. see @ref uart_bus_t
 * @else
 * @brief  去初始化，然后清除在 @ref hal_uart_regs_init 中设置的寄存器地址
 * @param  [in]  bus 串口号， 参考 @ref uart_bus_t
 * @endif
 */
