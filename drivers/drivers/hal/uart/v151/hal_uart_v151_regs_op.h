...
/**
 * @brief  Attempts to claim a UART
...
/**
 * @brief  Determines if a UART is claimed
...
#if CORE == BT
/**
 * @brief  Used to select whith core uart interrupt signal should be connect to.
 * @param  uart The UART bus will set.
 * @param  core The core that select uart interrupt connect to.
 * @return None.
 */
void hal_uart_int_config(uart_bus_t uart, cores_t core);

/**
 * @brief  Used to select whith core uart dma handshaking should be connect to.
 * @param  uart The UART bus will set.
 * @param  core The core that select uart dma connect to.
 * @return None.
 */
void hal_uart_dma_config(uart_bus_t uart, cores_t core);
#endif
...
/**
 * @brief  Enables the uart for transmission, reception and or cts.
 * @param  uart Uart bus.
...
/**
 * @brief  Registers the interrupt callback associated with a interrupt type.
 * @param  uart Uart bus.
 ...
 */
void hal_uart_configure_interrupt(uart_bus_t uart, hal_uart_interrupt_t interrupt_type,
                                  isr_callback callback, interrupt_state_t initial_state);
...
/**
 * @brief  Returns the raw interrupt status of the uart.
 * @param  uart
 * @return the raw interrupt status of the uart.
 */
uint32_t hal_uart_get_raw_isr_status(uart_bus_t uart);

/**
 * @brief  config uart dma tx and rx.
 * @param  uart Uart bus.
 * @param  config dma rx and tx config
 */
...
/**
 * @brief  Fill the error status into err_info_data
 * @param  uart The uart needs to store err status.
 * @param  err_info_data The data array to save.
 * @param  length The length of err_info_data.
 * @param  uart_err_int_flg The uart error status data.
 */
 ...
 * @param uart The uart needs to set dma
