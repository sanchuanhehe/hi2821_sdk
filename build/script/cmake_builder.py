...
class CMakeBuilder(BuildEnvironment):
    ...
    def get_build_cmd(self, env):
        ...
        for component in self.component:
            if component in env.get('ram_component', False):
                ext_cmd.append(component)
            if component in env.get('ram_component_set', False):
    ...
    def build_sdk(self, env):
        ...
        for sdk_target in sdk_pkg_target_name:
            sdk_target_env = TargetEnvironment(sdk_target)
    ...
    def start(self, env, target_name, output_path, nhso=None, clean=None):
        ...
        if self.is_command_refresh(output_path):
            ...
            _excute(["cmake", "--build", output_path, '-j%d' % self.thread], log_file, self.dump)

        if len(self.component) > 0:
            os.chdir(org_work_path)
            print("######### Build target:%s, component:[%s] success" % (target_name, ' '.join(self.component)))
            return 0
    ...
    def pack_fwpkg(self, chip, target_name):
        ...
        print("packet ing...")
        errcode = exec_shell([self.python_path, packet_script_path, chip, target_name], None, True)
        if errcode != 0:
            print("packet error!")
            self.print_build_result(target_name, errcode)
            sys.exit(1)
