...
def fill_fota_image_secure_head(upg_bin, fota_image, cf, image_confg_key, image_bin):
    ...
    upg_bin[16:48] = tmp_hash[0:32]  # 填充fota_image.image_hash
    if cf.get(image_confg_key, 'ProtectionKeyL1') != "":
        data_byte1 = (hex2dec(cf.get(image_confg_key, 'ProtectionKeyL1'))).to_bytes(16, byteorder=sys.byteorder.capitalize().lower(), signed=True)
        upg_bin[fota_image_head_size - 52:fota_image_head_size - 36] = data_byte1  # 填充fota_image.enc_pk_l1
    if cf.get(image_confg_key, 'ProtectionKeyL2') != "":
        data_byte2 = (hex2dec(cf.get(image_confg_key, 'ProtectionKeyL2'))).to_bytes(16, byteorder=sys.byteorder.capitalize().lower(), signed=True)
        upg_bin[fota_image_head_size - 36:fota_image_head_size - 20] = data_byte2  # 填充fota_image.enc_pk_l2
...
def fill_fota_image(cf, image_confg_key, all_image_bin, fota_obj, start_offset, image_offset):
    ...
    fota_image.image_len = image_len
...
def make_upg(fota_obj):
    ...
    sign_suite = int(cf.get('SIGN_CFG', 'SignSuite'))
    # FOTA Key Area 填充
...
def create_fota_file(user_input):
    ...
    # 检查nv是否与APP一起的打包
    check_nv_bin(fota_obj.input_dict)
    # 生成镜像配置文件
