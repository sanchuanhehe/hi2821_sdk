...
def database_create():
    ...
    target_cus = G_PARAMS["DATABASE_VERSION"].split()
    for cus in set(target_cus):
        database_target = '_'.join(['database', cus])
        # 指定database目录
        database_target = os.path.join(G_PARAMS["OUT_DIR"], database_target)
        ...
        for core in cores:
            ...
            if core == 'acore':
                try:
                    path = os.rename(os.path.join(database_target, core), os.path.join(database_target, 'cco'))
                except:
